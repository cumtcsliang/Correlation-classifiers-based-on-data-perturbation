function [W2,ww,Weight2,ob]=maxminCorraltion1(X,y,Para1,Para2, LabelMatrix,y1,Max_iter)
      %  Max_iter=10
         %Para1=1;
         %Para2=1;
         %X=rand(10,5);
...
for i=1:max(y)
            ...
            ff=rand(1,size(dd,1));
              InitalWeight1  =[InitalWeight1 ff/sum(ff)];
   ...
   for i=1:size(X,2)
    for j=1:size(X,2)
        ...
        u=1*(max(1)*norm(K.*K1/Para1+K1/Para3,Inf)+Para2);
           ...
           %  InitalWeight=InitalWeight/size(X,2);
            ...
            for iter=1:Max_iter
            ob(iter,1)=(r'*(K.*K1)*r)/(2*Para1)+(r'*(K1)*r)/2+real(Para2* sum((r'+1e-20).*log((r'+1e-20)./(InitalWeight)))) ; 
            ...
            tttt=K.*K1*r/Para1+K1*r+real(Para2*(log(max(r,0)+1e-10)+1'-log((InitalWeight)')));
             ...
             for jj=1:max(y)
                 ...
                 sss=temptemp1(dd,1);
                temptemp2=[temptemp2;sss/sum(sss)];
             end
